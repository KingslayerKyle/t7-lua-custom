...
CoD.T8AmmoContainer.new = function ( menu, controller )
	...
	self.WeaponImage:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "currentWeapon.viewmodelWeaponName" ), function ( modelRef )
		if Engine.GetModelValue( modelRef ) then
			if Engine.GetModelValue( modelRef ):find( "^" .. "pistol_" )
			or Engine.GetModelValue( modelRef ):find( "^" .. "ar_" )
			or Engine.GetModelValue( modelRef ):find( "^" .. "smg_" )
			or Engine.GetModelValue( modelRef ):find( "^" .. "shotgun_" )
			or Engine.GetModelValue( modelRef ):find( "^" .. "lmg_" )
			or Engine.GetModelValue( modelRef ):find( "^" .. "sniper_" ) then
				if Engine.GetModelValue( modelRef ):find( "_upgraded" ) then
					self.WeaponImage:setImage( RegisterImage( "t7_icon_weapon_" .. tostring( Engine.GetModelValue( modelRef ) ):gsub( "_upgraded_zm", "_pu" ) ) )
				else
					self.WeaponImage:setImage( RegisterImage( "t7_icon_weapon_" .. tostring( Engine.GetModelValue( modelRef ) ):gsub( "_zm", "_pu" ) ) )
				end
			else
				self.WeaponImage:setImage( RegisterImage( "blacktransparent" ) )
			end
	...
	self.ShieldMeterBG:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "hudItems.showDpadDown" ), function ( modelRef )
		if Engine.GetModelValue( modelRef ) == 1 then
	...
	self.MuleKickIcon:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "T8.MuleKick" ), function ( modelRef )
		if Engine.GetModelValue( modelRef ) then
			if Engine.GetModelValue( modelRef ) == 1 then
	...
	self.AATIcon:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "CurrentWeapon.aatIcon" ), function ( modelRef )
		if Engine.GetModelValue( modelRef ) then
			self.AATIcon:setImage( RegisterImage( Engine.GetModelValue( modelRef ) ) )
		end
	end )
