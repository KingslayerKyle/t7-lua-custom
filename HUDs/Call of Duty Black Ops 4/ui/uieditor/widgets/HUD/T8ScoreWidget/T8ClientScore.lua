...
CoD.T8ClientScore.new = function ( menu, controller )
	...
	self.ScoreTextGlow:linkToElementModel( self, "clientNum", true, function ( modelRef )
		if Engine.GetModelValue( modelRef ) then
			self.ScoreTextGlow:setRGB( ZombieClientScoreboardColor( Engine.GetModelValue( modelRef ) ) )
		end
	end )
