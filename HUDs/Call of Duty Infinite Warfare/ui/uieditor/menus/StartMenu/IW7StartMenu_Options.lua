...
local PostLoadFunc = function ( self, controller )
	...
	if CoD.isCampaign then
		self:setModel( Engine.CreateModel( Engine.GetModelForController( controller ), "IW7StartMenu_Options" ) )
	end
...
LUI.createMenu.IW7StartMenu_Options = function ( controller )
	local self = CoD.Menu.NewForUIEditor( "IW7StartMenu_Options" )
	...
	self.buttonModel = Engine.CreateModel( Engine.GetModelForController( controller ), "IW7StartMenu_Options.buttonPrompts" )
	...
	self.TabFrame:linkToElementModel( self.FETabBar.Tabs.grid, "tabWidget", true, function ( model )
		if Engine.GetModelValue( model ) then
			self.TabFrame:changeFrameWidget( Engine.GetModelValue( model ) )
		end
	end )
	...
	LUI.OverrideFunction_CallOriginalSecond( self, "close", function ( element )
		...
		Engine.UnsubscribeAndFreeModel( Engine.GetModel( Engine.GetModelForController( controller ), "IW7StartMenu_Options.buttonPrompts" ) )
	end )
