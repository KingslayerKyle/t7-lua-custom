...
CoD.CSGOSelfScore.new = function ( menu, controller )
	...
	self.ZoneText:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "csgo_zone_name" ), function ( modelRef )
	...
	self.PortraitImage:linkToElementModel( self, "zombiePlayerIcon", true, function ( modelRef )
		if Engine.GetModelValue( modelRef ) then
			if Engine.GetModelValue( modelRef ) == "blacktransparent" then
				self.PortraitImage:setImage( RegisterImage( "csgo_player_none" ) )
			else
	...
	self.PortraitImageFrame:linkToElementModel( self, "clientNum", true, function ( modelRef )
		if Engine.GetModelValue( modelRef ) then
			self.PortraitImageFrame:setRGB( ZombieClientScoreboardGlowColor( Engine.GetModelValue( modelRef ) ) )
		end
	end )
	...
	self.HealthText:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "CSGO.Health" ), function ( modelRef )
		if Engine.GetModelValue( modelRef ) then
	...
	self.HealthBar:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "CSGO.Health" ), function ( modelRef )
		if Engine.GetModelValue( modelRef ) then
	...
	self.ArmorText:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "ZMInventory.shield_health" ), function ( modelRef )
		if Engine.GetModelValue( modelRef ) then
			local armorInt = math.floor( ( Engine.GetModelValue( modelRef ) * 100 ) + 0.5 )
			
			if Engine.GetModelValue( Engine.GetModel( Engine.GetModelForController( controller ), "hudItems.showDpadDown" ) ) ~= nil then
				if Engine.GetModelValue( Engine.GetModel( Engine.GetModelForController( controller ), "hudItems.showDpadDown" ) ) > 0 then
					if armorInt < 0 then
						self.ArmorText:setText( Engine.Localize( "0" ) )
					else
	...
	self.ArmorBar:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "ZMInventory.shield_health" ), function ( modelRef )
		if Engine.GetModelValue( modelRef ) then
			if Engine.GetModelValue( Engine.GetModel( Engine.GetModelForController( controller ), "hudItems.showDpadDown" ) ) ~= nil then
				if Engine.GetModelValue( Engine.GetModel( Engine.GetModelForController( controller ), "hudItems.showDpadDown" ) ) > 0 then
			...
			if Engine.GetModelValue( modelRef ) <= 1 / 3 then
				self.ArmorBar:setRGB( 1, 0.11, 0.11 )
			else
				self.ArmorBar:setRGB( 1, 1, 1 )
			end
