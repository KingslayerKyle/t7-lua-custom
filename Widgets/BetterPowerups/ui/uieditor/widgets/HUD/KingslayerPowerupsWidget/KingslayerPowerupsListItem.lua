...
CoD.KingslayerPowerupsListItem.new = function ( menu, controller )
	...
	self.image:linkToElementModel( self, "image", true, function ( modelRef )
		if Engine.GetModelValue( modelRef ) then
			self.image:setImage( RegisterImage( Engine.GetModelValue( modelRef ) ) )
		end
	end )
	...
	self:linkToElementModel( self, "state", true, function ( modelRef )
